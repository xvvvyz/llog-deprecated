  ...
  useEffect(() => {
    (async () => {
      ...
      for (const d of data) {
        ...
        if (d.RecordedBy) {
          (availableFieldValues.RecordedBy as Set<string>).add(d.RecordedBy);
        }

        if (d.InputLabel) {
          (availableFieldValues.InputLabel as Set<string>).add(d.InputLabel);
        }

        if (!inputTypes[d.InputLabel]) {
          inputTypes[d.InputLabel] = /(select|multi_select|checkbox)/.test(
            d.InputType,
          )
            ? 'nominal'
            : 'quantitative';
      ...
      for (const [key, value] of Object.entries(availableFieldValues)) {
        if (value instanceof Set) availableFieldValues[key] = Array.from(value);
        else availableFieldValues[key] = value;
      }
      ...
      setMessages([
        ...
        {
          content: 'Hello! I can create visualizations for your data.',
          id: nanoid(),
          role: 'assistant',
        },
      ]);
    })();
  }, [dataRef, setMessages, subjectId]);
            ...
            {messages.map((m) => {
              ...
              if (func) {
                ...
                if (!dataRef.current || !Object.keys(args).length) {
                  return (
                    <div
                      className="border-t border-alpha-1 px-4 pt-4 text-left font-mono text-xs text-fg-3"
                      key={m.id}
                    >

    ...
    defaultValues: useDefaultValues({
      ...
      defaultValues: {
        ...
        sessions: sessions.map((s) => {
          ...
          return {
            ...session,
            modules: forceArray(session?.modules).map((module) => {
              eventsMap[module.id] = firstIfArray(module.event);

              return {
                ...module,
                inputs: forceArray(module?.inputs).reduce(
                  (acc, { input_id }) => {
    ...
    <form
      ...
      onSubmit={form.handleSubmit(async (values) => {
        ...
        const { insertedEventTypes, updatedEventTypes } = form
          ...
          .reduce(
            (acc, session) => {
              session.modules.map((module, order) => {
        ...
        const deletedEventTypeIds = sessions.reduce((acc, s) => {
          ...
          forceArray(session?.modules).forEach((module) => {
